...
def landing():
    with get_db() as db:
        ...
        for kid in kids:
            ...
            stars_today = db.execute('SELECT COUNT(*) FROM task_instances WHERE user_id = ? AND starred = 1 AND date = ?',
                               (kid['id'], today_str)).fetchone()[0]
            kid_dict['stars_month'] = stars_month
            kid_dict['stars_today'] = stars_today
...
def home():
    ...
    with get_db() as db:
        templates = db.execute('SELECT * FROM task_templates ORDER BY category').fetchall()
        if not templates:
...
def settings():
    ...
    if request.method == 'POST':
        ...
        with get_db() as db:
            if action == 'update_passcode':
                ...
                passcode = request.form['passcode']
                if name and user_type and len(passcode) == 4 and passcode.isdigit():
                ...
                if name and category in ['morning', 'evening', 'night']:
                    db.execute('INSERT INTO task_templates (name, category) VALUES (?, ?)', (name, category))
                    db.commit()
    ...
    with get_db() as db:
        ...
        tasks = db.execute('SELECT * FROM task_templates ORDER BY category').fetchall() if is_parent else None
